...
class LaserTracker:
    ...
    def update_hsv_from_web(self, color, h_low=None, h_high=None, s_low=None, s_high=None, v_low=None, v_high=None):
        """从网页更新 HSV 值"""
        with self.lock:
            if color == 'black':
                if h_low is not None: self.hsv_values['black']['low'][0] = h_low
                if h_high is not None: self.hsv_values['black']['high'][0] = h_high
                if s_low is not None: self.hsv_values['black']['low'][1] = s_low
                if s_high is not None: self.hsv_values['black']['high'][1] = s_high
                if v_low is not None: self.hsv_values['black']['low'][2] = v_low
                if v_high is not None: self.hsv_values['black']['high'][2] = v_high
            elif color == 'red':
                if h_low is not None:
                    self.hsv_values['red1']['low'][0] = h_low
                    self.hsv_values['red2']['low'][0] = h_low
                if h_high is not None:
                    self.hsv_values['red1']['high'][0] = h_high
                    self.hsv_values['red2']['high'][0] = h_high
                if s_low is not None:
                    self.hsv_values['red1']['low'][1] = s_low
                    self.hsv_values['red2']['low'][1] = s_low
                if s_high is not None:
                    self.hsv_values['red1']['high'][1] = s_high
                    self.hsv_values['red2']['high'][1] = s_high
                if v_low is not None:
                    self.hsv_values['red1']['low'][2] = v_low
                    self.hsv_values['red2']['low'][2] = v_low
                if v_high is not None:
                    self.hsv_values['red1']['high'][2] = v_high
                    self.hsv_values['red2']['high'][2] = v_high
            elif color == 'green':
                if h_low is not None: self.hsv_values['green']['low'][0] = h_low
                if h_high is not None: self.hsv_values['green']['high'][0] = h_high
                if s_low is not None: self.hsv_values['green']['low'][1] = s_low
                if s_high is not None: self.hsv_values['green']['high'][1] = s_high
                if v_low is not None: self.hsv_values['green']['low'][2] = v_low
                if v_high is not None: self.hsv_values['green']['high'][2] = v_high
    ...
    def process_frame(self, frame):
        ...
        with self.lock:
            ...
            self.black_mask_display = cv2.resize(black_mask, (width, height))
            self.red_mask_display = cv2.resize(red_mask, (width, height))
            self.green_mask_display = cv2.resize(green_mask, (width, height))
...
@app.route('/update_hsv')
def update_hsv():
    """更新 HSV 值的路由"""
    color = request.args.get('color')
    h_low = request.args.get('h_low', type=int)
    h_high = request.args.get('h_high', type=int)
    s_low = request.args.get('s_low', type=int)
    s_high = request.args.get('s_high', type=int)
    v_low = request.args.get('v_low', type=int)
    v_high = request.args.get('v_high', type=int)

    tracker.update_hsv_from_web(color, h_low, h_high, s_low, s_high, v_low, v_high)
    return jsonify({'status': 'success'})

@app.route('/get_hsv_values')
def get_hsv_values():
    """返回当前的 HSV 值"""
    with tracker.lock:
        # 将 numpy 数组转换为普通列表
        hsv_values = {
            'black': {
                'low': tracker.hsv_values['black']['low'].tolist(),
                'high': tracker.hsv_values['black']['high'].tolist()
            },
            'red1': {
                'low': tracker.hsv_values['red1']['low'].tolist(),
                'high': tracker.hsv_values['red1']['high'].tolist()
            },
            'red2': {
                'low': tracker.hsv_values['red2']['low'].tolist(),
                'high': tracker.hsv_values['red2']['high'].tolist()
            },
            'green': {
                'low': tracker.hsv_values['green']['low'].tolist(),
                'high': tracker.hsv_values['green']['high'].tolist()
            }
        }
...
def save_hsv():
    """保存 HSV 值到 JSON 文件"""
    try:
        tracker.save_hsv_to_json(tracker.hsv_config_path)
        return jsonify({'status': 'success', 'message': 'HSV values saved successfully'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500


@app.route('/mask_feed/<color>')
def mask_feed(color):
    """遮罩视频流路由"""
    ...
    def generate_mask():
        while tracker.running:
            mask = None
            with tracker.lock:
                if color == 'black' and hasattr(tracker, 'black_mask_display'):
                    mask = tracker.black_mask_display
                elif color == 'red' and hasattr(tracker, 'red_mask_display'):
                    mask = tracker.red_mask_display
                elif color == 'green' and hasattr(tracker, 'green_mask_display'):
                    mask = tracker.green_mask_display

            if mask is not None:
                # 转换为 3 通道以便显示
                mask_display = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
                _, buffer = cv2.imencode('.jpg', mask_display, [cv2.IMWRITE_JPEG_QUALITY, 50])
                frame_bytes = buffer.tobytes()
                yield (b'--frame\r\n'
                       b'Content-Type: image/jpeg\r\n\r\n' + frame_bytes + b'\r\n')
